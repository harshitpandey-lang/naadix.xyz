...
.github/workflows/pages.yml builds, checks and uploads only dist, then deploys using the official Pages artifact workflow. In repository Settings → Pages, select GitHub Actions as the publishing source. Preserve the existing naadix.xyz custom domain. Do not use branch-root publishing. The workflow needs Pages enabled and environment permissions. See [GitHub's custom workflow documentation](https://docs.github.com/en/pages/getting-started-with-github-pages/using-custom-workflows-with-github-pages).
