...
await emit("404.html", layout("/404.html", "Signal Lost — NaadiX", "The system you are looking for is not connected here.", `<section class="section prose"><p class="eyebrow">404 / SIGNAL LOST</p><h1>Signal<br><span>lost.</span></h1><p class="lede">The system you're looking for isn't connected here.</p><div class="hero-actions"><a class="button primary" href="/">Return Home ↗</a><a class="text-link" href="/labs/">Explore NaadiX Labs ↗</a></div></section>`).replace("</head>", '<meta name="robots" content="noindex"></head>'));
...
for (const [old, target] of Object.entries(redirects)) await emit(old.slice(1), `<!doctype html><html lang="en"><head><meta charset="utf-8"><meta name="viewport" content="width=device-width,initial-scale=1"><title>Page moved — NaadiX</title><meta http-equiv="refresh" content="0;url=${target}"><link rel="canonical" href="${company.domain}${target}"></head><body><p>This page has moved. <a href="${target}">Continue to NaadiX</a>.</p></body></html>`);
await emit("_redirects", Object.entries(redirects).map(([a, b]) => `${a} ${b} 301`).join("\n") + "\n");
await emit("_headers", `/*\n  X-Content-Type-Options: nosniff\n  Referrer-Policy: strict-origin-when-cross-origin\n  Permissions-Policy: camera=(), geolocation=()\n  Content-Security-Policy: default-src 'self'; script-src 'self' https://static.cloudflareinsights.com; style-src 'self' 'unsafe-inline'; img-src 'self' data:; connect-src 'self' https://oruezuyfjavhjnadullk.supabase.co https://cloudflareinsights.com https://*.cloudflareinsights.com; frame-ancestors 'none'; base-uri 'self'; form-action 'self' mailto:; object-src 'none'\n`);
