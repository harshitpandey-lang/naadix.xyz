...
if (out !== join(root, "dist")) throw Error("Refusing an unexpected output directory");
await rm(out, { recursive: true, force: true });
await mkdir(join(out, "assets"), { recursive: true });
const emit = async (path, body) => {
  const target = resolve(out, path);
  if (!target.startsWith(out + "/") && !target.startsWith(out + "\\")) throw Error("Invalid output path");
...
await mkdir(join(out, "assets/hq/whisper"), { recursive: true });
for (const file of ["libstream.js", "libstream.worker.js"]) {
  await cp(join(root, "node_modules/whisper-web-transcriber/dist", file), join(out, "assets/hq/whisper", file));
}
await cp(join(root, "node_modules/whisper-web-transcriber/LICENSE"), join(out, "assets/hq/whisper/LICENSE.txt"));
const hqPages = [
...
await emit(".nojekyll", "");
const redirects = {
  "/webpages/home.html": "/", "/home.html": "/", "/webpages/mission.html": "/method/", "/mission.html": "/method/", "/webpages/FAQ.html": "/faq/", "/webpages/contact-us.html": "/contact/", "/webpages/contact.html": "/contact/", "/contact.html": "/contact/", "/webpages/founder/intro.html": "/founder/", "/founder.html": "/founder/", "/webpages/founder/skills.html": "/founder/", "/webpages/founder/contact-me.html": "/contact/", "/webpages/founder/event-gallery.html": "/founder/", "/webpages/products/home-product.html": "/labs/", "/projects.html": "/labs/", "/webpages/founder/projects/home-projects.html": "/labs/", "/webpages/products/content-gen-agent.html": "/labs/content-agent/", "/webpages/products/affiliate-agent.html": "/labs/affiliate-agent/", "/webpages/founder/projects/content-gen-agent.html": "/labs/content-agent/", "/webpages/founder/projects/contentagent.html": "/labs/content-agent/", "/webpages/founder/projects/affiliate-agent.html": "/labs/affiliate-agent/", "/webpages/founder/projects/affiliateagent.html": "/labs/affiliate-agent/", "/webpages/founder/projects/rover.html": "/labs/",
};
for (const [old, target] of Object.entries(redirects)) await emit(old.slice(1), `<!doctype html><html lang="en"><head><meta charset="utf-8"><meta name="viewport" content="width=device-width,initial-scale=1"><title>Page moved â€” NaadiX</title><meta http-equiv="refresh" content="0;url=${target}"><link rel="canonical" href="${company.domain}${target}"></head><body><p>This page has moved. <a href="${target}">Continue to NaadiX</a>.</p></body></html>`);
await emit("_redirects", Object.entries(redirects).map(([a, b]) => `${a} ${b} 301`).join("\n") + "\n");
const whisperCsp = "default-src 'self'; script-src 'self' 'unsafe-eval' https://static.cloudflareinsights.com; worker-src 'self'; style-src 'self' 'unsafe-inline'; img-src 'self' data:; connect-src 'self' https://oruezuyfjavhjnadullk.supabase.co https://cloudflareinsights.com https://*.cloudflareinsights.com https://huggingface.co https://*.huggingface.co https://*.hf.co https://*.xethub.hf.co; frame-ancestors 'none'; base-uri 'self'; form-action 'self' mailto:; object-src 'none'";
await emit("_headers", `/*\n  X-Content-Type-Options: nosniff\n  Referrer-Policy: strict-origin-when-cross-origin\n  Permissions-Policy: camera=(), microphone=(self), geolocation=()\n  Content-Security-Policy: default-src 'self'; script-src 'self' https://static.cloudflareinsights.com; worker-src 'self'; style-src 'self' 'unsafe-inline'; img-src 'self' data:; connect-src 'self' https://oruezuyfjavhjnadullk.supabase.co https://cloudflareinsights.com https://*.cloudflareinsights.com https://huggingface.co https://*.huggingface.co https://*.hf.co https://*.xethub.hf.co; frame-ancestors 'none'; base-uri 'self'; form-action 'self' mailto:; object-src 'none'\n\n/hq/meetings/*\n  Cross-Origin-Opener-Policy: same-origin\n  Cross-Origin-Embedder-Policy: require-corp\n  Content-Security-Policy: ${whisperCsp}\n\n/assets/hq/whisper/*\n  Cross-Origin-Resource-Policy: same-origin\n  Cross-Origin-Opener-Policy: same-origin\n  Cross-Origin-Embedder-Policy: require-corp\n  Content-Security-Policy: ${whisperCsp}\n  Cache-Control: public, max-age=31536000, immutable\n\n/assets/hq/whisper-*\n  Cross-Origin-Resource-Policy: same-origin\n  Cross-Origin-Opener-Policy: same-origin\n  Cross-Origin-Embedder-Policy: require-corp\n  Content-Security-Policy: ${whisperCsp}\n`);
await emit("webpages/lab/sw-lab.js", `self.addEventListener('install',()=>self.skipWaiting());\nself.addEventListener('activate',event=>event.waitUntil((async()=>{for(const key of await caches.keys()){if(key.startsWith('naadix-lab-'))await caches.delete(key);}await self.registration.unregister();})()));\n`);
const width = 1200, height = 630, raw = Buffer.alloc((width * 4 + 1) * height);
...
for (let y = 0; y < height; y++) for (let x = 0; x < width; x++) { const i = y * (width * 4 + 1) + 1 + x * 4, d = Math.hypot(x - 900, y - 315), glow = Math.max(0, 1 - d / 350), grid = x % 60 === 0 || y % 60 === 0; let r = 5 + glow * 10, g = 7 + glow * 17, b = 12 + glow * 40; if (grid) { r += 8; g += 8; b += 9; } if (segment(x, y, 110, 425, 110, 205) < 5 || segment(x, y, 110, 205, 330, 425) < 5 || segment(x, y, 330, 425, 330, 205) < 5 || segment(x, y, 110, 315, 330, 315) < 3) { r = 164; g = 188; b = 255; } for (let n = 0; n < 6; n++) { const a = (n * Math.PI) / 3, px = 900 + 190 * Math.cos(a), py = 315 + 190 * Math.sin(a); if (segment(x, y, 900, 315, px, py) < 1.2 || Math.hypot(x - px, y - py) < 6) { r = 110; g = 145; b = 212; } } if (d < 10) { r = 210; g = 223; b = 255; } raw[i] = r; raw[i + 1] = g; raw[i + 2] = b; raw[i + 3] = 255; }
const crc = (buf) => { let c = 0xffffffff; for (const b of buf) { c ^= b; for (let i = 0; i < 8; i++) c = (c >>> 1) ^ (c & 1 ? 0xedb88320 : 0); } return (c ^ 0xffffffff) >>> 0; };
const chunk = (name, data) => { const type = Buffer.from(name), body = Buffer.concat([type, data]), size = Buffer.alloc(4), sum = Buffer.alloc(4); size.writeUInt32BE(data.length); sum.writeUInt32BE(crc(body)); return Buffer.concat([size, body, sum]); };
