import { addDays, dateKey, esc, formatDate, formatDateTime, goalProgress, isGoalAtRisk, isStaleProject, startOfDay } from "./core.js";
...
async function load(shell) {
  ...
  try {
    const [projects, goals, events, actions] = await Promise.all([
      supabase.query("projects", { select: "id,name,status,priority,health,deadline,next_action,progress,updated_at", order: "updated_at.desc", limit: 40 }),
      supabase.query("goals", { select: "id,title,due_date,scheduled_start,completed,target_value,current_value,project_id", order: "due_date.asc", limit: 40 }),
...
function render(shell, { projects, goals, events, actions }) {
  ...
  const attention = [...goals.filter((goal) => !goal.completed && (!goal.due_date || goal.due_date < dateKey(addDays(today, 1)) || isGoalAtRisk(goal, goalProgress(goal)))).map((goal) => ({ label: goal.due_date && goal.due_date < dateKey(today) ? "Overdue goal" : "Goal at risk", title: goal.title, href: `/hq/goals/?goal=${goal.id}` })), ...projects.filter((project) => project.status === "ACTIVE" && (project.health === "BLOCKED" || isStaleProject(project) || (project.deadline && project.deadline <= dateKey(addDays(today, 3))))).map((project) => ({ label: project.health === "BLOCKED" ? "Blocked project" : isStaleProject(project) ? "Stale project" : "Project deadline", title: project.name, href: `/hq/projects/?project=${project.id}` }))];
  shell.content.innerHTML = `<section class="summary-grid dashboard-summary" aria-label="Workspace summary"><a href="/hq/projects/"><span>Active projects</span><strong>${activeProjects.length}</strong><small>${projects.length} total</small></a><a href="/hq/goals/"><span>Open goals</span><strong>${openGoals.length}</strong><small>${openGoals[0]?.due_date ? `Next due ${formatDate(openGoals[0].due_date)}` : "No upcoming due date"}</small></a><a href="/hq/calendar/"><span>Today's schedule</span><strong>${events.length}</strong><small>${events[0] ? `Next ${formatDateTime(events[0].start_at)}` : "Calendar is clear"}</small></a></section>
    <div class="dashboard-grid"><section class="dashboard-panel span-2"><header><div><p class="eyebrow">Attention</p><h2>What needs you now</h2></div></header>${attention.length ? `<div class="dashboard-list">${attention.slice(0, 6).map((item) => `<a href="${item.href}"><span><strong>${esc(item.title)}</strong><small>${item.label}</small></span></a>`).join("")}</div>` : '<p class="subtle panel-empty">Nothing urgent.</p>'}</section><section class="dashboard-panel span-2"><header><div><p class="eyebrow">In motion</p><h2>Active projects</h2></div><a href="/hq/projects/">View all</a></header>${activeProjects.length ? `<div class="dashboard-list">${activeProjects.slice(0, 5).map((project) => `<a href="/hq/projects/?project=${project.id}"><span><strong>${esc(project.name)}</strong><small>${project.next_action || (project.deadline ? `Due ${formatDate(project.deadline)}` : "No next action")}</small></span><span class="progress-compact"><i><b style="width:${project.progress ?? 0}%"></b></i>${project.progress ?? 0}%</span></a>`).join("")}</div>` : '<p class="subtle panel-empty">No active projects.</p>'}</section>
