import { GOAL_TYPES, completionPayload, esc, formatDate, formatDateTime, goalMatches, localInputValue, validateGoal } from "./core.js";
...
const state = { goals: [], query: "", filter: "ACTIVE", type: "ALL" };
let shell;
...
async function load() {
  ...
  try {
    state.goals = await supabase.query("goals", { select: "id,title,description,goal_type,target_value,unit,due_date,scheduled_start,scheduled_end,completed,completed_at,created_at,updated_at", order: "due_date.asc", limit: 250 });
    renderWorkspace();
...
function renderWorkspace() {
  ...
  shell.content.innerHTML = `<section class="summary-grid" aria-label="Goal summary"><article><span>Active goals</span><strong>${active}</strong></article><article><span>Completed</span><strong>${completed}</strong></article><article><span>Due in 7 days</span><strong>${dueSoon}</strong></article><article><span>Overall completion</span><strong>${completion}%</strong></article></section>
    <section class="workspace-controls" aria-label="Goal controls"><label class="search-control">${icon("search")}<span class="sr-only">Search goals</span><input type="search" placeholder="Search goals" value="${esc(state.query)}" data-goal-search></label><div class="filter-tabs" role="group" aria-label="Filter goal status">${[["ACTIVE","Active"],["DUE_SOON","Due soon"],["COMPLETED","Completed"],["ALL","All"]].map(([value,label]) => `<button type="button" data-goal-filter="${value}" class="${state.filter === value ? "active" : ""}">${label}</button>`).join("")}</div><label class="select-control"><span class="sr-only">Filter goal type</span><select data-goal-type><option value="ALL">All types</option>${GOAL_TYPES.map((type) => `<option value="${type}" ${state.type === type ? "selected" : ""}>${type[0].toUpperCase() + type.slice(1)}</option>`).join("")}</select></label></section><div id="goal-results"></div>`;
...
function renderResults() {
  ...
  const goals = state.goals.filter((goal) => goalMatches(goal, { query: state.query, state: state.filter, type: state.type }));
