...
async function load() {
  ...
  try {
    ...
    if (params.get("status") === "BLOCKED") { state.status = "BLOCKED"; renderWorkspace(); }
    if (params.get("new") === "1") { history.replaceState(null, "", "/hq/projects/"); openProjectForm(); return; }
...
async function renderDetail(dialog, project) {
  ...
  try {
    const [items, actions, decisions, waiting] = await Promise.all([
      ...
      supabase.query("decisions", { select: "id,title,decision,decided_at,project_id", filters: { project_id: `eq.${project.id}` }, order: "decided_at.desc", limit: 100 }),
      supabase.query("waiting_items", { select: "id,title,waiting_for,follow_up_at,status,related_project_id", filters: { related_project_id: `eq.${project.id}`, status: "eq.WAITING" }, order: "created_at.asc", limit: 100 }),
    ]);
    ...
    content.innerHTML = `<div class="detail-meta"><span class="status-pill" data-status="${project.status || "PLANNED"}">${statusLabels[project.status] || "Planning"}</span><span class="priority-mark">P${project.priority ?? 3}</span><span class="health-mark" data-health="${project.health || "ON_TRACK"}">${(project.health || "ON_TRACK").replaceAll("_", " ")}</span><span>${progress}% complete</span><span>${project.deadline ? `Due ${formatDate(project.deadline)}` : "No deadline"}</span></div>
      ...
      <section class="detail-section"><div class="section-heading"><div><p class="eyebrow">Related goals</p><h3>Outcomes</h3></div><a class="quiet-button compact" href="/hq/goals/?new=1&project=${project.id}">${icon("plus")}New goal</a></div>${relatedGoals.length ? `<div class="detail-list">${relatedGoals.map((goal) => `<a class="detail-row" href="/hq/goals/?goal=${goal.id}"><strong>${esc(goal.title)}</strong>${icon("chevron")}</a>`).join("")}</div>` : '<p class="subtle">No related goals yet.</p>'}</section>
      <section class="detail-section"><div class="section-heading"><div><p class="eyebrow">Decisions</p><h3>Why we chose this</h3></div><a class="quiet-button compact" href="/hq/decisions/?new=1&project=${project.id}">${icon("plus")}Record decision</a></div>${decisions.length ? `<div class="detail-list">${decisions.map((item) => `<a class="detail-row" href="/hq/decisions/?decision=${item.id}"><div><strong>${esc(item.title)}</strong><p>${esc(item.decision)} · ${formatDate(item.decided_at)}</p></div>${icon("chevron")}</a>`).join("")}</div>` : '<p class="subtle">No decisions recorded for this project.</p>'}</section>
      <section class="detail-section"><div class="section-heading"><div><p class="eyebrow">Waiting on</p><h3>Dependencies</h3></div><a class="quiet-button compact" href="/hq/dashboard/?waiting=new&project=${project.id}#waiting">${icon("plus")}Waiting on</a></div>${waiting.length ? `<div class="detail-list">${waiting.map((item) => `<a class="detail-row" href="/hq/dashboard/?waiting=show#waiting"><div><strong>${esc(item.title)}</strong><p>Waiting for ${esc(item.waiting_for)}${item.follow_up_at ? ` · Follow up ${formatDate(item.follow_up_at)}` : ""}</p></div>${icon("chevron")}</a>`).join("")}</div>` : '<p class="subtle">No open dependencies.</p>'}</section>
