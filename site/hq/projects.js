import { PROJECT_CATEGORIES, PROJECT_STATUSES, esc, formatDate, formatDateTime, projectMatches, slugify, validateProject } from "./core.js";
...
const state = { projects: [], actions: [], query: "", status: "ALL", view: sessionStorage.getItem("hq_project_view") || "list" };
let shell;
...
async function load() {
  ...
  try {
    [state.projects, state.actions] = await Promise.all([
      supabase.query("projects", { select: "id,name,slug,short_description,category,status,priority,progress,deadline,github_url,notes,overview,created_at,updated_at", order: "updated_at.desc", limit: 200 }),
...
function renderTable(projects) {
  return `<div class="table-shell"><table class="data-table project-table"><thead><tr><th>Project</th><th>Status</th><th>Priority</th><th>Progress</th><th>Next action</th><th>Deadline</th><th><span class="sr-only">Open</span></th></tr></thead><tbody>${projects.map((project) => {
    const action = nextAction(project.id);
    return `<tr><td><button class="table-primary" type="button" data-open-project="${project.id}"><strong>${esc(project.name)}</strong><span>${esc(project.category || "Uncategorized")}</span></button></td><td><span class="status-pill" data-status="${project.status || "PLANNED"}">${statusLabels[project.status] || "Planning"}</span></td><td><span class="priority-mark">P${project.priority ?? 3}</span></td><td><div class="progress-cell"><span>${project.progress ?? 0}%</span><i><b style="width:${Math.max(0, Math.min(100, project.progress ?? 0))}%"></b></i></div></td><td>${action ? `<span class="next-action">${esc(action.task)}${action.due_date ? `<small>${formatDate(action.due_date)}</small>` : ""}</span>` : '<span class="subtle">No open action</span>'}</td><td>${formatDate(project.deadline)}</td><td><button class="icon-button" type="button" data-open-project="${project.id}" aria-label="Open ${esc(project.name)}">${icon("chevron")}</button></td></tr>`;
