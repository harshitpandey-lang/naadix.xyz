...
export function layout(path, title, description, body) {
 ...
 <header class="header"><a class="brand" href="/" aria-label="NaadiX home"><span class="brand-mark" aria-hidden="true"><img src="/assets/naadix-logo.png" alt=""></span><span class="brand-word">NAADIX</span></a><nav class="desktop-nav" aria-label="Primary">${navigation.map(([t, h]) => `<a href="${h}" ${path === h ? 'aria-current="page"' : ""}>${t}</a>`).join("")}</nav><div class="nav-actions"><a class="hq-link" href="${c.hq}/login" aria-label="Open Founder HQ" title="Founder HQ">HQ <span aria-hidden="true">↗</span></a><button class="command-trigger" aria-label="Open command menu" title="Ctrl or Command K">⌘ K</button>${link("Discuss a project", "/contact/", "button small")}<button id="menu-toggle" aria-expanded="false" aria-controls="mobile-menu">Menu</button></div></header>
 <nav id="mobile-menu" aria-label="Mobile" hidden>${link("Home", "/")}${navigation.map(([t, h]) => link(t, h)).join("")}${link("Founder HQ", `${c.hq}/login`)}${link("Contact", "/contact/")}</nav>
